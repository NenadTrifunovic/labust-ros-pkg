...
class Estimator {
	...
	struct FilterState {
		...
		vector input;
		vector measurement;
		vector measurement_new;/* Value -1 denotes no measurement, values >=0 denote measurement delay. */ ///nije dobra ideja
		vector measurement_delay;
		vector measurement_offset;
		vector measurement_angle_unwrapping;
		vector state;
		matrix p_covariance;
		matrix r_covariance;
	};
...
public:
	...
	void setMeasurementVector(measurement_vector measurement,
			measurement_vector new_measurement);
...
private:
	...
	void checkOutliers(measurement_vector measurement,
			measurement_vector new_measuremen);
